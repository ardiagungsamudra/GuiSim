 ...
 * @author user
 */
public class MTI0800Participant implements TransactionParticipant {
    List<FieldModel> lf = new ArrayList<FieldModel>();
    
    public MTI0800Participant(List list) {
        lf = list;
    }
    ...
    public void commit(long id, Serializable context) {
        ...
        ISOMsg msg = (ISOMsg) ((Context)ctx).get("REQUEST");

        try {
            msg.setResponseMTI();
            for(int i = 0; i < lf.size(); i++){
                if(lf.get(i).getStatus())   // set if status field is true
                    msg.set(Integer.parseInt(lf.get(i).getField().toString()), lf.get(i).getValue());
                 else
                    msg.unset(Integer.parseInt(lf.get(i).getField().toString())); //unset if status field is false
            }
            msg.set(39, "00");
